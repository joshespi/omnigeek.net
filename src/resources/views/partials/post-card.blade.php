    ...
    <div class="flex items-center justify-between mb-2">
        <div class="flex items-center gap-2">
            @if ($post->user->avatar_url)
                <img src="{{ $post->user->avatar_url }}" class="h-9 w-9 rounded-full object-cover" alt="" />
            @else
                <div class="h-9 w-9 rounded-full bg-brand-600 text-white text-sm flex items-center justify-center font-semibold">
                    {{ $post->user->initials() }}
                </div>
            @endif
            <div>
