...
- **Email subscribe** (`/subscribe`) — double opt-in, token-based confirm/unsubscribe (no login). Scope by any mix of categories/geeks/tags (JSON `filters`, match ANY; empty = everything). Publishing queues `NotifySubscribersOfNewPost` to confirmed matches. `channel`/`frequency` columns are seams for future digests/push/Discord — only instant email wired. Needs a mailer (dev: `MAIL_MAILER=log` → `storage/logs/laravel.log`) and a queue worker.
